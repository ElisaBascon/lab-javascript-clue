...
function selectRandom() {
  let randomSuspects = suspectsArray[Math.floor(Math.random() * suspectsArray.length)]
  let randomRooms = roomsArray[Math.floor(Math.random() * roomsArray.length)]
  let randomWeapons = weaponsArray [Math.floor(Math.random() * weaponsArray .length)]

  return(randomSuspects, randomRooms, randomWeapons)
};
